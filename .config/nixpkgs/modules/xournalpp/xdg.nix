...
in {
  xdg.configFile = {
    "xournalpp/settings.xml" = {
      source = config.lib.file.mkOutOfStoreSymlink /. + "${paths.modules}/xournalpp/config/settings.xml";
      target = "xournalpp/settings.xml";
    };
    "xournalpp/colornames.ini" = {
      source = config.lib.file.mkOutOfStoreSymlink /. + "${paths.modules}/xournalpp/config/colornames.ini";
      target = "xournalpp/colornames.ini";
    };
    "xournalpp/toolbar.ini" = {
      source = config.lib.file.mkOutOfStoreSymlink /. + "${paths.modules}/xournalpp/config/toolbar.ini";
